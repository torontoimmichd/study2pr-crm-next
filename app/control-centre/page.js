...
const SECTIONS = [
  ...
  {
    key: 'subcategories', group: 'sell', label: 'Sub-categories', singular: 'sub-category',
    ...
    note: 'Use Destinations on a row to say which countries offer it. Leave it empty and it is offered everywhere.',
  },
  {
  ...
  {
    key: 'visas', group: 'sell', label: 'Visa types and permits', singular: 'visa type',
    data: 'visa_categories', table: 'visa_categories', gate: 'edit_catalogue',
    cols: [['country_id', 'Destination', (v, r, c) => nameOf(c.countries, v)],
           ['category_id', 'Category', (v, r, c) => nameOf(c.categories, v)],
           ['subcategory_id', 'Sub-category', (v, r, c) => nameOf(c.subcategories, v)],
           ['name', 'Visa or permit'], ['is_active', 'Active']],
    fn: 'fn_cc_save_visa_category',
    addHint: 'Named by where it is filed and what kind of work it is.',
    fields: [
      {k: 'country', label: 'Destination country', p: 'p_country', required: true, from: 'country_id',
       opts: {from: 'countries', where: c => c.is_active}},
      {k: 'category', label: 'Category', p: 'p_category', from: 'category_id',
       opts: {from: 'categories', where: c => c.is_active}},
      {k: 'subcategory', label: 'Sub-category', p: 'p_subcategory', from: 'subcategory_id',
       opts: {from: 'subcategories', where: s => s.is_active}},
      {k: 'name', label: 'Visa or permit', required: true, hint: 'e.g. Study permit, Visitor visa (TRV)'},
      {k: 'code', label: 'Code'},
  ...
  {
    key: 'programmes', group: 'sell', label: 'Programmes', singular: 'programme',
    data: 'programmes', table: 'programmes', gate: 'edit_catalogue',
    cols: [['name', 'Programme'], ['code', 'Code'], ['family', 'Family'], ['is_active', 'Active']],
    fn: 'fn_cc_save_programme',
    addHint: 'The filing route behind a visa type. Rules, documents and fees hang off its versions.',
    fields: [
      {k: 'visa_category_id', label: 'Visa type', opts: {from: 'visa_categories'}},
      {k: 'name', label: 'Programme', required: true},
      {k: 'code', label: 'Code'},
      {k: 'family', label: 'Family', hint: 'Study, Work, PR…'},
      {k: 'report_label', label: 'Report label'},
      {k: 'summary', label: 'Summary', type: 'area', wide: true},
      {k: 'is_active', label: 'Active', type: 'bool'},
    ],
  },
  ...
  {
    key: 'library', group: 'answer', label: 'Document library', singular: 'document',
    data: 'checklist_documents', table: 'checklist_documents', gate: 'edit_catalogue',
    cols: [['name', 'Document'], ['code', 'Code'], ['note', 'Note'], ['is_active', 'Active']],
    fn: 'fn_cc_save_checklist_document',
    addHint: 'A document you ask clients for. Requirements decide when it is asked for.',
    fields: [
...
export default function ControlCentre() {
  ...
  const load = useCallback(async () => {
    ...
    const {data, error: e} = await supabase.rpc('fn_cc_config');
    if (e) { setError(e.message); setState('error'); return; }
    setConfig(data ?? {});
    setState('ready');
  }, []);
  ...
  return (<div className="page">
    ...
    {dest
      ? <Destinations row={dest} config={config} save={save}
...
function Section({spec, rows, hidden, config, can, onAdd, onEdit, onRowAction, showRetired, setShowRetired}) {
  ...
  return (<>
    <div className="card-h" style={{border: 0, padding: '0 0 12px'}}>
      ...
      {editable
        ...
        : <span className="tag t-mute">READ ONLY FOR YOU</span>}
    </div>
          ...
          <tbody>{rows.map(r => <tr key={r.id}>
            ...
            <td style={{textAlign: 'right', whiteSpace: 'nowrap'}}>
              {editable && spec.rowAction
                ...
                : null}
              {editable ? <button className="mini" onClick={() => onEdit(r)}>Edit</button> : null}</td>
          </tr>)}</tbody>
        </table></div></div>}
