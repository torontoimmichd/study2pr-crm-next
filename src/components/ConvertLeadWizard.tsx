...
export function ConvertLeadWizard({ lead, open, onOpenChange, onConverted }: Props) {
  ...
  useEffect(() => {
    ...
    const ordered = [...familyLeads].sort((a, b) => {
      ...
      const aPrimary = a.family_role === "primary";
      const bPrimary = b.family_role === "primary";
  ...
  const handleConvert = async () => {
    ...
    if (orderedFamilyLeads.length > 1 && applicants.length !== orderedFamilyLeads.length) {
      toast.error(`This family has ${orderedFamilyLeads.length} applicants. Please keep all family applicants in the application.`);
      return;
    }
    if (orderedFamilyLeads.length > 1 && applicants.some(a => !a.name.trim() || !a.visaTypeId)) {
      toast.error("Every family applicant needs a name and visa type");
      return;
    }
    ...
    try {
      ...
      const isFamilyConversion = orderedFamilyLeads.length > 1 && applicants.length > 1;
      const conversionClients: { client: { id: string; family_unit_id?: string | null }; leadId: string; applicant: Applicant }[] = [
        { client, leadId: lead.id, applicant: applicants[0] },
      ];
      if (isFamilyConversion) {
        for (let i = 1; i < orderedFamilyLeads.length; i += 1) {
          const memberLead = orderedFamilyLeads[i];
          const member = applicants[i];
          const { data: existingMember } = await supabase
            .from("clients")
            .select("id, family_unit_id")
            .eq("source_lead_id", memberLead.id)
            .maybeSingle();
          let memberClient = existingMember;
          if (!memberClient) {
            const { data: createdMember, error: memberError } = await supabase
              ...
              .select("id, family_unit_id")
              .single();
            if (memberError || !createdMember) throw new Error(memberError?.message ?? `Could not create client for ${member.name}`);
      ...
      const casePayloads = conversionClients.map(({ client: memberClient, applicant }, index) => ({
        ...
        is_archived: false,
        payment_plan_enabled: index === 0 && stagesEnabled,
        payment_stages: index === 0 && stagesEnabled ? cleanStages : null,
      }));
      const { data: caseRows, error: caseErr } = await (supabase as any)
        .from("cases")
        .insert(casePayloads)
        .select("id, case_code, client_id");
      if (caseErr || !caseRows?.length) { toast.error(caseErr?.message ?? "Failed to create case"); setBusy(false); return; }
      const caseByClient = new Map<string, { id: string; case_code: string | null }>(caseRows.map((row: { id: string; case_code: string | null; client_id: string }) => [row.client_id, row]));
      const primaryCase = caseByClient.get(client.id) ?? caseRows[0];
      const caseRef = primaryCase.case_code ?? primaryCase.id.slice(0, 8);
      ...
      for (const entry of conversionClients) {
        ...
        void writeAudit({ action: "CREATE", entity_type: "clients", entity_id: entry.client.id, changes: { from_lead: entry.leadId } });
        void writeAudit({ action: "CREATE", entity_type: "cases", entity_id: createdCase.id, changes: { case_code: createdCase.case_code } });
        void createCaseTasks(createdCase.id, caseManager || profile?.id || null, user?.id || null);
        if (entry.applicant.notes.trim()) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          void (supabase as any).from("entity_notes").insert({ case_id: createdCase.id, client_id: entry.client.id, note_type: "general", body: entry.applicant.notes.trim(), is_locked: false, created_by: user?.id ?? null });
        }
      }
      void writeTimeline({ event_type: "lead_converted", title: `Lead converted — ${caseRows.length > 1 ? `${caseRows.length} family applications` : `Case ${caseRef} created`}`, lead_id: lead.id, case_id: primaryCase.id, client_id: client.id, is_system: false });
