...
// NOTE (P1.5): expiry_alert_rules / expiry_items are absent from the generated
// src/integrations/supabase/types.ts, so .from() resolves to `never`. Cast per the
// existing codebase convention (105 other call sites). Proper fix = regenerate
// types.ts from the live schema — deferred to Phase 3 (see PHASE1_REPORT).
...
export default function AdminExpiry() {
  ...
  const { data: rules } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("expiry_alert_rules")
  ...
  const { data: items } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("expiry_items")
  ...
  const addItem = async () => {
    ...
    setBusy(true);
    const { error } = await (supabase as any).from("expiry_items").insert({
  ...
  const toggleItem = async (it: Item, on: boolean) => {
    const { error } = await (supabase as any).from("expiry_items").update({ is_active: on }).eq("id", it.id);
    if (error) toast.error(error.message); else refresh();
  };

  const deleteItem = async () => {
    if (!deleting) return;
    const { error } = await (supabase as any).from("expiry_items").delete().eq("id", deleting.id);
    if (error) { toast.error(error.message); return; }
    setDeleting(null); refresh();
  };

  const toggleRule = async (r: Rule, on: boolean) => {
    const { error } = await (supabase as any).from("expiry_alert_rules")
